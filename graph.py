...
def to_graph_app(graph_name, out, nodes):
    out_file = out
    ...
    for filename, file_data in nodes.items():
        ...
        net.add_node(
            ...
            # imports_datas = file_data["imports"],
            exports = ''.join([f"<div classname=\"panel-export-symbol\">{expname}</div>" for expname, exp in file_data["exports"].items()]),
            exports_names = ''.join([f"<div class='symb' style='width: 80%;' id='symb-{expname}' style='margin-bottom: 10px; display: inline-block; user-select: none;'><b>{file_data['filename']} :</b> {expname}</div>" for expname, exp in file_data["exports"].items()])
        )
    ...
    <script type="text/javascript">
        maskedNodes = []
        function unMaskNode(nodeId) {
            maskedNodes.splice(maskedNodes.indexOf(nodeId), 1)
            network.body.data.nodes.update({
                id: nodeId,
                hidden: false
            });
            elt = document.getElementById("unmask-"+nodeId)
            elt.remove()
        }
        function updateMaskedNodesPanel() {
            // Display the node details in the node panel
            elt = document.getElementById("maskedNodesPanel")
            elt.innerHTML  = "<div style='width: 100%; border-bottom: 2px solid lightgreen;'><b style='font-size: 1.5rem;'>Masked nodes:</b></div><br>"
            maskedNodes.forEach(function (nodeId) {
                nodeId_txt = '"'+nodeId+'"'
                elt.innerHTML += "<div id='unmask-"+nodeId+"' style='margin-bottom: 10px; display: inline-block; user-select: none;'><div style='display:inline-block; background-color: lightgreen; border-radius: 5px;' onclick='unMaskNode("+nodeId_txt+")'>unmask</div><b style='font-size: 0.8rem;'>" + nodeId + "</b></div>"
            });
        }
        function maskNode(nodeId) {
            maskedNodes.push(nodeId)
            network.body.data.nodes.update({
                id: nodeId,
                hidden: true
            });
            updateMaskedNodesPanel()
        }
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            // Add masked nodes right panel
            var maskedNodesPanel = document.createElement("div");
            maskedNodesPanel.id = "maskedNodesPanel";
            maskedNodesPanel.style.position = "absolute";
            maskedNodesPanel.style.top = "0px";
            maskedNodesPanel.style.right = "0px";
            maskedNodesPanel.style.width = "25vw";
            maskedNodesPanel.style.height = "30vh";
            maskedNodesPanel.style.overflow = "scroll";
            maskedNodesPanel.style.padding = "0px 10px 10px 10px";
            maskedNodesPanel.style.backgroundColor = "rgba(255, 255, 255, 0.9)";
            maskedNodesPanel.style.border = "1px solid #ccc";
            maskedNodesPanel.style.display = "block"; 

            // Add all symbols panel
            var symbolsPanel = document.createElement("div");
            symbolsPanel.id = "symbolsPanel";
            symbolsPanel.style.position = "absolute";
            symbolsPanel.style.top = "30vh";
            symbolsPanel.style.right = "0px";
            symbolsPanel.style.width = "25vw";
            symbolsPanel.style.height = "70vh";
            symbolsPanel.style.overflow = "scroll";
            symbolsPanel.style.padding = "0px 10px 10px 10px";
            symbolsPanel.style.backgroundColor = "rgba(255, 255, 255, 0.9)";
            symbolsPanel.style.border = "1px solid #ccc";
            symbolsPanel.style.display = "block";  

            let symbolsPanel_symsHTML    = "<div style='width: 100%; position: sticky; top: 0px; background-color: white; padding-bottom: 10px;'><div style='width: 100%; border-bottom: 2px solid lightgreen; margin-bottom: 10px;'><b style='font-size: 1.5rem;'>Symbols :</b></div>"
            symbolsPanel_symsHTML       += "<div style='width: 80%; margin: auto'><input style='width: 100%;' type='text' id='symb-search' placeholder='Search for a symbol...'/><br></div></div>"
            symbolsPanel_symsHTML       += "<div style='overflow: scroll'>"
            network.body.data.nodes.forEach(function (node) {
                symbolsPanel_symsHTML += node.exports_names
            });
            symbolsPanel_symsHTML       += "</div>"
            symbolsPanel.innerHTML      += symbolsPanel_symsHTML

            document.body.appendChild(nodePanel);
            document.body.appendChild(importsPanel);
            document.body.appendChild(exportsPanel);
            document.body.appendChild(maskedNodesPanel);
            document.body.appendChild(symbolsPanel);

            // Récupérer l'input de recherche et les éléments de la liste
            const searchInput = document.getElementById('symb-search');
            const symbs = document.querySelectorAll('.symb');

            // Ajouter un écouteur d'événement pour détecter les entrées de l'utilisateur
            searchInput.addEventListener('input', () => {
                const searchTerm = searchInput.value.toLowerCase();
                // Parcourir chaque élément pour vérifier s'il correspond à la recherche
                symbs.forEach(symb => {
                if (symb.id.toLowerCase().includes(searchTerm)) {
                        symb.style.display = 'block'; // Afficher l'élément
                    } else {
                        symb.style.display = 'none'; // Masquer l'élément
                    }
                });
            });
            ...
            function resetAll() {
                ...
                edgePanel.style.display = "none";  // Show the panel
            ...
            function redArrowEdges(edgesId) {
            ...
            network.on("selectNode", function (params) {
                var nodeId = params.nodes[0]; // Get the selected node
                var nodeData = network.body.data.nodes.get(nodeId);
                var nodeId_txt = '"'+nodeId+'"'
                // Display the node details in the node panel
                nodePanel.innerHTML = "<div style='width: 100%; border-bottom: 2px solid lightgreen;'><b style='font-size: 1.5rem;'>File details:</b></div><br>" + 
                                    "<div style='position: absolute; top: 10px; background-color: red; right: 10px; border-radius: 5px;' onclick='maskNode("+nodeId_txt+")'>Mask node</div>" + 
                ...
                resetEdges()
                redArrowEdges(connectedEdges)
            });

            network.on("selectEdge", function (params) {
                redArrowEdges(params.edges)
            });
            ...
            network.on("click", function (params) {
                ...
                if (params.nodes.length > 0) {
                    ...
                    // Select adjacent edges of the clicked node
                    var connectedEdges = network.getConnectedEdges(nodeId);
